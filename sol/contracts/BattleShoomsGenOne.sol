...
contract BattleShroomsGenOne is ERC721Enumerable, Ownable {
    ...
    function addWL(address _es) public onlyOwner() {
        _whiteListed[_es] = true;
    }

    function removeWL(address _es) public onlyOwner() {
        _whiteListed[_es] = false;
    }

    function addWLMany(address[] memory _wls) public onlyOwner() {
        require(totalSupply() + _wls.length <= _presaleReserved, 'Would exceed ES reserved supply');
